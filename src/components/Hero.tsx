...
export const Hero: React.FC<HeroProps> = ({ onOpenConsultation }) => {
  ...
  return (
    <section className="relative min-h-screen w-full overflow-hidden sky-hero-backdrop flex items-center justify-center select-none pt-24 pb-12 lg:pt-0 lg:pb-0">
      <div className="max-w-7xl mx-auto px-6 lg:px-12 w-full grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-8 items-center z-10">
        ...
        >
          ...
          <h1 className="tracking-tight text-[#1A1A1A] leading-[1.06] text-left">
            <span className="font-serif italic font-normal block text-4xl sm:text-6xl lg:text-7xl xl:text-[76px] text-[#1A1A1A]">
              Sensible Disruption
            </span>
            <span className="font-sans font-black block text-3xl sm:text-5xl lg:text-6xl xl:text-[62px] text-[#1A1A1A] mt-1">
          ...
          <div className="w-full max-w-lg pt-2">
            <form onSubmit={handleSubmit} className="relative flex items-center rounded-full bg-white/55 backdrop-blur-xl border border-white/80 shadow-2xl p-1.5 transition-all focus-within:bg-white/85 focus-within:shadow-sky-300/40">
              <input
                ...
                className="w-full h-12 sm:h-14 pl-5 sm:pl-6 pr-28 sm:pr-36 bg-transparent text-[#1A1A1A] placeholder:text-[#475E80] text-sm sm:text-base font-semibold focus:outline-none"
              />
              <button
                type="submit"
                className="absolute right-1.5 top-1.5 bottom-1.5 bg-white/95 hover:bg-white text-[#1A1A1A] hover:text-[#0284C7] border border-white px-4 sm:px-6 rounded-full text-xs font-bold uppercase tracking-wider flex items-center gap-2 transition-all duration-200 hover:scale-[1.03] active:scale-95 shadow-md shrink-0"
              >
        ...
        {/* RIGHT COLUMN: Lady Justice Cutout (Scaled slightly smaller for breathing room) */}
        <motion.div
        ...
        >
          <div className="relative w-full max-w-[240px] sm:max-w-[300px] lg:max-w-[360px] aspect-[2/3] flex items-center justify-center">
